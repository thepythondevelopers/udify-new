...
export class StoresComponent implements OnInit {
  stores: any = [];
  constructor(public api: ApiService, private ngxService: NgxUiLoaderService) {}
  ...
  sync(type: any, store: any) {
    ...
    // this.api.get('shopify-sync-node/sync-' + type + '/' + store._id, {}).subscribe(
    this.api.get(type + '-node/sync-' + type + '/' + store._id, {}).subscribe(
      async (data: any) => {
        if (data.hasOwnProperty('error')) {
  ...
  deleteStore(store: any) {
    ...
    // this.api.showSwal('success', 'The store will get updated with the current details of the shopify.');
    // this.api.get('shopify-sync-node/sync-' + type + '/' + store._id, {}).subscribe(
    this.api.delete('integration-node/delete-integration/' + store._id, {}).subscribe(
      async (data: any) => {
        if (data.hasOwnProperty('error')) {
          this.api.showToast('error', data.error);
          return false;
